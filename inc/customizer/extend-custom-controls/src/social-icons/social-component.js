...
const SocialComponent = props => {
	...
	const is_sharing_mode = props?.control?.params?.share_mode
		? props.control.params.share_mode
		: "";
	...
	const site_social_icon = [
		...
		{value: 'twitter-X', label: __('Twitter-X', 'astra'), color: '#7acdee', background: 'transparent'},
	]
			...
			<ReactSortable animation={100} onStart={() => onDragStop()} onEnd={() => onDragStop()}
				...
				{currentList.length > 0 && currentList.map((item, index) => {
					return <ItemComponent removeItem={remove => removeItem(remove)}
										  ...
										  is_sharing_mode={is_sharing_mode}
										  />;
