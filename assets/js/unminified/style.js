...
(function () {
	...
	AstraToggleSubMenu = function () {
		...
		for (var j = 0; j < parent_li_sibling.length; j++) {
			if (parent_li_sibling[j] != parent_li) {
				...
				var all_sub_menu =
					parent_li_sibling[j].querySelectorAll(".sub-menu");
	...
	AstraToggleSetup = function () {
		...
		if (menu_toggle_all.length > 0) {
			for (var i = 0; i < menu_toggle_all.length; i++) {
				...
				if (!nav_menu_click_listeners[i]) {
					nav_menu_click_listeners[i] = menu_toggle_all[i];
				...
				if ("undefined" !== typeof __main_header_all[i]) {
					if (
						...
						var astra_menu_toggle = __main_header_all[
							i
						].querySelectorAll(
	...
	astraNavMenuToggle = function (event) {
		...
		for (var i = 0; i < menuHasChildren.length; i++) {
			...
			var menuHasChildrenSubMenu =
				menuHasChildren[i].querySelectorAll(".sub-menu");
		...
		if (menu_class.indexOf("main-header-menu-toggle") !== -1) {
			...
			if (
				__main_header_all[event_index].classList.contains("toggle-on")
			) {
		...
		function () {
			...
			if (__main_header_all.length > 0) {
				for (var i = 0; i < __main_header_all.length; i++) {
					...
					var searchIcons = __main_header_all[
						i
					].getElementsByClassName("ast-search-menu-icon");
		...
		SearchIcons[i].onclick = function (event) {
			if (this.classList.contains("slide-search")) {
				...
				if (!sibling.classList.contains("ast-dropdown-active")) {
					...
					var searchTerm =
						sibling.querySelector(".search-field").value || "";
					if ("" !== searchTerm) {
		...
		if (typeof event.target.classList !== "undefined") {
			if (
				...
				for (var i = 0; i < dropdownSearchWrap.length; i++) {
					dropdownSearchWrap[i].classList.remove(
						"ast-dropdown-active"
					);
				}
			}
	...
	/**
	 * Resets the header menu state.
	 *
	 * @param {HTMLElement} beforeToggle - The element representing the toggle.
	 * @param {HTMLElement} barNavigation - The element representing the bar navigation.
	 * @param {HTMLElement} bar - The element representing the bar.
	 * @since 1.3.2
	 * @return {void}
	 */
	function resetHeaderMenu(beforeToggle, barNavigation, bar) {
		beforeToggle.classList.remove("toggled");
		barNavigation.classList.remove("toggle-on");
		...
		if (self && !self.classList.contains("astra-search-icon")) {
			...
			if (link.indexOf(hash) !== -1) {
				...
				if (
					...
					var main_header_menu_toggle = document.querySelector(
					...
					var before_header_menu_toggle = document.querySelector(
						".menu-below-header-toggle"
					);
					var before_header_bar_navigation =
						document.querySelector(".ast-below-header");
					var before_header_bar = document.querySelector(
						".ast-below-header-actual-nav"
					);

					if (
						before_header_menu_toggle &&
						before_header_bar_navigation &&
						before_header_bar
					) {
						before_header_menu_toggle.classList.remove("toggled");
						before_header_bar_navigation.classList.remove(
							"toggle-on"
						);
					...
					var after_header_menu_toggle = document.querySelector(
						".menu-above-header-toggle"
					);
					var after_header_bar_navigation =
						document.querySelector(".ast-above-header");
					var after_header_bar = document.querySelector(
						".ast-above-header-navigation"
					);

					if (
						after_header_menu_toggle &&
						after_header_bar_navigation &&
						after_header_bar
					) {
						after_header_menu_toggle.classList.remove("toggled");
						after_header_bar_navigation.classList.remove(
							"toggle-on"
						);
	 ...
	 * @param {HTMLElement} element - The element to toggle the focus class on.
	 */
	function toggleFocusClass(element) {
		// On li elements toggle the class .focus.
		if ("li" === element.tagName.toLowerCase()) {
			if (-1 !== element.className.indexOf("focus")) {
	...
	if (astra.is_scroll_to_id) {
		...
		function scrollToIDHandler(e) {
			...
			if (siteHeader) {
				...
				const headerHeight = siteHeader.querySelectorAll(
